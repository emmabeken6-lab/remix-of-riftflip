...
export const playCoinflip = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { error: debitErr } = await supabaseAdmin.rpc("apply_transaction", {
      ...
      _ref_id: game.id, _meta: { game: "coinflip", side: data.side, server_seed_hash: serverSeedHash },
    });
    if (debitErr) {
    ...
    await supabaseAdmin.from("games").update({
      status: "resolved",
      result: { result, won, payout, server_seed: serverSeed, client_seed: clientSeed, nonce } as never,
